...
async function sendSlotConfirmationEmail(
  ...
  try {
    ...
    console.error("[confirm-interview-slot] Failed to send slot confirmation email:", e);
  }
}
...
async function verifyToken(selectionId: string, slot: string, token: string): Promise<boolean> {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (isNaN(scheduledAt.getTime())) {
      return new Response(
        JSON.stringify({ error: "Invalid slot date" }),
        { status: 400, headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" } }
      );
    }

    // Enforce 2-day expiry for selection links based on selection timestamps
    ...
    if (scheduledAt.getTime() < minTime.getTime()) {
      return new Response(
        JSON.stringify({ error: "This time slot is less than 10 minutes from now. Please choose a later slot." }),
        { status: 400, headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" } }
      );
    }

    if (!(await verifyToken(selectionId, slot, token))) {
      return new Response(
        JSON.stringify({ error: "Invalid or expired link" }),
        { status: 403, headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" } }
      );
