...
- **Email OTP (passwordless)** for HR: `/signup` (creates user), `/login` (existing users only), `/auth/verify-otp`.
...
1. **Authentication → Providers → Email**: enable; configure **OTP** / magic link as you prefer.
2. **URL configuration**: add your site URL and redirect URLs (e.g. `http://localhost:5173` for Vite dev).
3. **Email templates**: ensure OTP emails are enabled so users receive codes.
